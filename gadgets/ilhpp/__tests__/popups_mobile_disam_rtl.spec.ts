...
test.use({ currentURL: `file://${import.meta.dirname}/link_disam_rtl.html`, isMobile: true });
...
testCombinations.forEach((combination) => {
  test.describe(`given ${JSON.stringify(combination)}`, () => {
    ...
    test('popup should have correct appearance', async ({ page }) => {
      ...
      await expect(page.getByText('查看相似页面')).toBeVisible();
      await expect(page).toHaveScreenshot();

      await page.getByTitle('关闭').click(); // Reset
      await expect(page.getByText('查看相似页面')).not.toBeVisible();
    });
  });
});
