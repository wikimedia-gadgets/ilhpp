...
test.use({ currentURL: `file://${import.meta.dirname}/link_long_linewrapped.html` });
...
testCombinations.forEach((combination) => {
  test.describe(`given ${JSON.stringify(combination)}`, () => {
    ...
    test('popup should have correct appearance and behaviors on line 1', async ({ page }) => {
      ...
      await expect(page.getByText('阅读更多内容')).toBeInViewport();
      await expect(anchor).not.toHaveAttribute('title');
      await expect(page).toHaveScreenshot();

      await page.mouse.move(0, 0); // Reset
      await expect(page.getByText('阅读更多内容')).not.toBeInViewport();
      await expect(anchor).toHaveAttribute('title');
    });

    test('popup should have correct appearance and behaviors on line 2', async ({ page }) => {
      const requestPromise = page.waitForResponse(/^https:\/\//);
      const anchor = page.locator('css=.ilh-page a');
      await expect(anchor).toHaveAttribute('title');
      await anchor.hover({
